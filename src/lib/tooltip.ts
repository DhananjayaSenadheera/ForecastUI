// Keeping a hover/focus tooltip inside the phone's screen.
//
...
//    hanging past the right edge of a 375px screen still counts towards the DOCUMENT's
//    scrollable width. /portfolio therefore laid out 457px wide with nothing visibly wrong,
//    the whole page could be dragged sideways, and (because a mobile viewport's initial
//    containing block grows with that overflow) `position: fixed; inset: 0` on the details
//    dialog's backdrop stretched to 457px too, carrying its ✕ off the screen. The fix is in
//    the stylesheet — `display: none` when closed — and it is pinned by
//    tooltip-overflow.test.ts, because jsdom applies no CSS and no behaviour test can see it.
//
...
//    that runs off the edge. CSS alone cannot express "shift left by however much you stick
//    out" (anchor positioning is far too new for a 2019 Android), so the shift is measured
//    here, in a handler, and written straight onto the node's style. Doing it imperatively in
//    the pointer/focus handler — not via React state — is deliberate: the browser has not
//    painted the hover yet when the handler runs, so the tooltip appears already clamped
//    instead of jumping into place a frame later (and never contributes an overflowing frame).
//
